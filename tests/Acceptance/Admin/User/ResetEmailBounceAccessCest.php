...
class ResetEmailBounceAccessCest
{
    private $pages = [
        '/admin/user/reset-email-bounce/',
    ];

    public function allowAccess(Tester $I): void
    {
        $testClosure = function (Tester $I) {
            $I->see('Разблокировать e-mail пользователя', 'h1');
        };
    ...
    {
        $testClosure = function (Tester $I) {
            $I->dontSee('Разблокировать e-mail пользователя', 'h1');
        };
